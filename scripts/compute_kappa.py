...
from oshc.agreement import with_ci
...
def main() -> int:
    ...
    for name, a, b in (("Gap", a_gap, b_gap), ("Code 1", a_code, b_code)):
        ...
        if weak:
            listed = ", ".join(f"{k} {v:.0%}" for k, v in sorted(weak.items()))
            print(f"  weakest categories : {listed}")
        print()

    disagreements = [
        {
            "id": i,
            "bikram_code1": master[i]["code1"],
            f"{args.second_coder}_code1": pack[i]["code1"],
            "bikram_gap": master[i]["gap"],
            f"{args.second_coder}_gap": pack[i]["gap"],
