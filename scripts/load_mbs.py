"""Build the MBS SQLite table from the official export.
    ...
    python scripts/load_mbs.py --source data/mbs/MBS-XML-2026.xml --db data/mbs/mbs.sqlite
...
from oshc.billexplainer.mbs import MbsRow, load_mbs_csv, normalise, to_sqlite  # noqa: E402
...
def load_xml(path: Path, captured_on: str) -> list[MbsRow]:
    ...
    for node in root.iter():
        ...
        try:
            ...
            skipped += 1
            continue
        rows.append(MbsRow(item.strip().upper(), desc, fee_dec,
                           _first(node, XML_FIELDS["category"]), captured_on))
    if not rows:
        raise ValueError(
...
def main() -> int:
    ...
    if src.suffix.lower() == ".xml":
        ...
        rows = [MbsRow(r.item_number, r.description, r.schedule_fee,
                       r.category, r.captured_on or args.captured_on)
